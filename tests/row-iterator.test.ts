...
Deno.test.beforeEach(() => {
  conn.execute(
    "CREATE TABLE items (id INTEGER PRIMARY KEY, title TEXT, price REAL)",
    [],
  );
...
Deno.test("iterator.next()", async (t) => {
  await t.step("should return IteratorResult with value and done", () => {
  ...
  await t.step("should return done=false for first row", () => {
  ...
  await t.step("should return done=true after final row", () => {
  ...
  await t.step("should provide row data in value", () => {
  ...
  await t.step("should iterate through all rows", () => {
  ...
  await t.step("should handle optional value parameter", () => {
...
Deno.test("Iterator as Iterable", async (t) => {
  await t.step("should work with for...of loop", () => {
  ...
  await t.step("should allow using spread operator", () => {
    conn.prepare("SELECT id FROM items ORDER BY id", (stmt) => {
      const rows = stmt.query([]);
      const iterator = rows.iterate();
...
Deno.test("Iterator with Data Validation", async (t) => {
  await t.step("should provide correct row structure", () => {
    conn.prepare(
      "SELECT id, title, price FROM items WHERE id = 1",
      (stmt) => {
        const rows = stmt.query([]);
        const iterator = rows.iterate();
        const result = iterator.next();
  ...
  await t.step("should preserve data types", () => {
    conn.prepare(
      "SELECT id, title, price FROM items WHERE id = 2",
      (stmt) => {
        const rows = stmt.query([]);
        const iterator = rows.iterate();
        const result = iterator.next();
  ...
  await t.step("should handle null values in row data", () => {
    conn.execute("INSERT INTO items (title, price) VALUES (?, ?)", [
      null,
      null,
    ]);
    ...
    conn.prepare(
      "SELECT title, price FROM items WHERE title IS NULL",
      (stmt) => {
        const rows = stmt.query([]);
        const iterator = rows.iterate();
        const result = iterator.next();
...
Deno.test("Iterator Edge Cases", async (t) => {
  ...
  await t.step("should handle single row", () => {
  ...
  await t.step("should handle sequential next() calls", () => {
  ...
  await t.step("should handle next() after exhaustion", () => {
    conn.prepare("SELECT * FROM items WHERE id > ?", (stmt) => {
      const rows = stmt.query([999]);
      const iterator = rows.iterate();
...
Deno.test("Iterator Helper Methods", async (t) => {
  await t.step("should have defined structure", () => {
    conn.prepare("SELECT * FROM items", (stmt) => {
      const rows = stmt.query([]);
      const iterator = rows.iterate();
  ...
  await t.step("should support iterator protocol", () => {
    conn.prepare("SELECT id FROM items ORDER BY id", (stmt) => {
      const rows = stmt.query([]);
      const iterator = rows.iterate();
