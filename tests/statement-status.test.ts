...
Deno.test("statement.getStatus()", async (t) => {
  await t.step("should read FullscanStep status", () => {
    let status = 0;
  ...
  await t.step("should read Sort status", () => {
    let status = 0;
  ...
  await t.step("should read AutoIndex status", () => {
    let status = 0;
    conn.prepare("SELECT * FROM numbers WHERE value = ?", (stmt) => {
      stmt.query([50]);
  ...
  await t.step("should read VmStep status", () => {
    let status = 0;
  ...
  await t.step("should read Run status", () => {
    let status = 0;
  ...
  await t.step("should read FilterMiss status", () => {
    let status = 0;
    conn.prepare(
      "SELECT * FROM numbers WHERE value > ? AND value < ?",
      (stmt) => {
        stmt.query([10, 90]);
  ...
  await t.step("should read FilterHit status", () => {
    let status = 0;
    conn.prepare(
      "SELECT * FROM numbers WHERE value > ? AND value < ?",
      (stmt) => {
  ...
  await t.step("should read MemUsed status", () => {
    let status = 0;
  ...
  await t.step("should return 0 before query execution", () => {
    let status = 0;
  ...
  await t.step("should increase after query execution", () => {
    let before = 0;
    let after = 0;
...
Deno.test("statement.resetStatus()", async (t) => {
  await t.step("should reset FullscanStep and return previous value", () => {
    let previousValue = 0;
    conn.prepare("SELECT * FROM numbers", (stmt) => {
      stmt.query([]);
  ...
  await t.step("should reset VmStep counter", () => {
    conn.prepare("SELECT * FROM numbers LIMIT 10", (stmt) => {
      stmt.query([]);
  ...
  await t.step("should reset Sort counter", () => {
    conn.prepare("SELECT * FROM numbers ORDER BY value DESC", (stmt) => {
      stmt.query([]);
  ...
  await t.step("should reset Run counter", () => {
    conn.prepare("SELECT COUNT(*) FROM numbers", (stmt) => {
      stmt.query([]);
  ...
  await t.step("should allow multiple resets", () => {
  ...
  await t.step("should work for all status types", () => {
    conn.prepare("SELECT * FROM numbers", (stmt) => {
      stmt.query([]);
...
Deno.test("RusqliteStatementStatus Enum", async (t) => {
  await t.step("should have documented status types", () => {
