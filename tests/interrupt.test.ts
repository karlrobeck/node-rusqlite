...
Deno.test("connection.getInterruptHandle()", async (t) => {
  await t.step("should return an InterruptHandle", () => {
    const handle = conn.getInterruptHandle();
    expect(handle).toBeDefined();
  });

  await t.step("should return object with interrupt method", () => {
    const handle = conn.getInterruptHandle();
    expect(typeof handle.interrupt).toBe("function");
  });

  await t.step("should allow multiple handle acquisitions", () => {
  ...
  await t.step("should return same type of handle for same connection", () => {
    const handle1 = conn.getInterruptHandle();
    const handle2 = conn.getInterruptHandle();
...
Deno.test("handle.interrupt()", async (t) => {
  ...
  await t.step("should set connection interrupt state", () => {
    const handle = conn.getInterruptHandle();
  ...
  await t.step(
    "should allow interrupting different handles independently",
    () => {
      const handle1 = conn.getInterruptHandle();
      const handle2 = conn.getInterruptHandle();
...
Deno.test("connection.isInterrupted()", async (t) => {
  await t.step("should return a boolean", () => {
    const interrupted = conn.isInterrupted();
    expect(typeof interrupted).toBe("boolean");
  });

  await t.step("should return false for non-interrupted connection", () => {
    const interrupted = conn.isInterrupted();
    expect(interrupted).toBe(false);
  });

  await t.step("should return true after interrupt()", () => {
    const handle = conn.getInterruptHandle();
    handle.interrupt();
  ...
  await t.step("should reflect interrupt state changes", () => {
...
Deno.test("Interrupt - Query Handling", async (t) => {
  await t.step("should allow querying after interrupt", () => {
    const handle = conn.getInterruptHandle();
  ...
  await t.step("should coexist with normal operations", () => {
    const handle1 = conn.getInterruptHandle();

    // Normal query
    const result1 = conn.queryRow(
...
Deno.test("Interrupt - Connection State", async (t) => {
  await t.step("should maintain interrupt state across operations", () => {
    const handle = conn.getInterruptHandle();
  ...
  await t.step("should allow checking interrupt from any handle", () => {
    const handle1 = conn.getInterruptHandle();
  ...
  await t.step("should work with multiple interrupt calls", () => {
    const handle = conn.getInterruptHandle();
...
Deno.test("Interrupt - Error Scenarios", async (t) => {
  await t.step("should handle interrupt on empty table", () => {
  ...
  await t.step("should handle interrupt on large result set", () => {
    const handle = conn.getInterruptHandle();
  ...
  await t.step("should handle rapid interrupt calls", () => {
    const handle = conn.getInterruptHandle();
...
Deno.test("Interrupt - Multiple Handles", async (t) => {
  await t.step(
    "should allow getting multiple handles from same connection",
    () => {
      const handles = [];
      for (let i = 0; i < 5; i++) {
  ...
  await t.step("should interrupt from any handle", () => {
    const handle1 = conn.getInterruptHandle();
    const handle2 = conn.getInterruptHandle();
  ...
  await t.step("should work with handles created at different times", () => {
    const handle1 = conn.getInterruptHandle();
